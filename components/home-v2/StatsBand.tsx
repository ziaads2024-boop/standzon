  ...
  return (
    <section className="border-y border-white/10 bg-[#141414] text-white">
      <div className="mx-auto grid max-w-[1400px] grid-cols-2 md:grid-cols-4">
        {stats.map((s, i) => (
          <div key={s.label} className={`px-6 py-14 md:px-10 md:py-20 ${i > 0 ? "md:border-l md:border-white/10" : ""} ${i % 2 === 1 ? "border-l border-white/10 md:border-l" : ""}`}>
            <div className="text-[clamp(2.5rem,5vw,4.5rem)] font-extralight leading-none tracking-[-0.04em]">
              <Count to={s.value} suffix={s.suffix} decimals={s.decimals} />
            </div>
            <div className="mt-4 text-[10px] font-semibold uppercase tracking-[0.25em] text-white/80">{s.label}</div>
          </div>
        ))}
      </div>
    </section>
